...
export default function AegisDashboard() {
  return (
    <div className="flex">
      <Sidebar />
      <div className="flex-1 min-h-screen bg-slate-950 text-slate-50 p-4 font-sans">
        {/* Header */}
        ...
        <main className="max-w-7xl mx-auto space-y-6">
          ...
          <section className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            <Link
              href="/report"
              className="flex items-center justify-center gap-2 bg-red-600 hover:bg-red-700 text-white font-semibold py-4 px-6 rounded-xl transition-all shadow-lg shadow-red-900/20"
            >
              <PlusCircle size={20} />
              Report Incident
            </Link>
            <Link
              href="/map"
              className="flex items-center justify-center gap-2 bg-slate-800 hover:bg-slate-700 text-white font-semibold py-4 px-6 rounded-xl border border-slate-700 transition-all"
            >
              <MapIcon size={20} />
              Open Live Map
            </Link>
                  ...
                  <tbody className="divide-y divide-slate-800">
                    {recentIncidents.map((incident) => (
                      <tr key={incident.id} className="hover:bg-slate-800/30 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          <span className={`px-2 py-1 rounded-full text-[10px] font-bold uppercase ${incident.severity === 'High' ? 'bg-red-500/10 text-red-500' :
                        ...
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-2 text-sm">
                            <span className={`w-2 h-2 rounded-full ${incident.status === 'Critical' ? 'bg-red-500 animate-pulse' :
